...
st:
    ...
    |  IF; ifE = expr; THEN; thenE = st; ELSE; elseE = st                    { {Ast.st_structure = Ast.IfS(ifE, thenE, elseE); st_data = Some(($startpos.Lexing.pos_lnum, $startpos.Lexing.pos_cnum - $startpos.Lexing.pos_bol + 1),
                                                                                                                   ($endpos.Lexing.pos_lnum, $endpos.Lexing.pos_cnum - $endpos.Lexing.pos_bol + 1),
                                                                                                                   $startpos.Lexing.pos_fname)} }
    |  mp = mid; LARROW; NEW; MAP; LESS; kt = ftype; COMMA; vt = ftype; esym = GREATER; SEMI; p = st     { {Ast.st_structure = Ast.FMapCreate(mp, kt, vt, p); st_data = Some(($startpos.Lexing.pos_lnum, $startpos.Lexing.pos_cnum - $startpos.Lexing.pos_bol + 1),
                                                                                                                   ($endpos(esym).Lexing.pos_lnum, $endpos(esym).Lexing.pos_cnum - $endpos(esym).Lexing.pos_bol + 1),
                                                                                                                   $startpos.Lexing.pos_fname)} }
    |  mp = mid; LARROW; NEW; MAP; LESS; kt = ftype; COMMA; vt = stype; esym = GREATER; SEMI; p = st     { {Ast.st_structure = Ast.STMapCreate(mp, kt, vt, p); st_data = Some(($startpos.Lexing.pos_lnum, $startpos.Lexing.pos_cnum - $startpos.Lexing.pos_bol + 1),
                                                                                                                   ($endpos(esym).Lexing.pos_lnum, $endpos(esym).Lexing.pos_cnum - $endpos(esym).Lexing.pos_bol + 1),
                                                                                                                   $startpos.Lexing.pos_fname)} }
    |  mp = mid; DOT; INSERT; LPAREN; k = expr; COMMA; v = expr; esym = RPAREN; SEMI; p = st     { {Ast.st_structure = Ast.FMapInsert(mp, k, v, p); st_data = Some(($startpos.Lexing.pos_lnum, $startpos.Lexing.pos_cnum - $startpos.Lexing.pos_bol + 1),
                                                                                                                   ($endpos(esym).Lexing.pos_lnum, $endpos(esym).Lexing.pos_cnum - $endpos(esym).Lexing.pos_bol + 1),
                                                                                                                   $startpos.Lexing.pos_fname)} }
    |  mp = mid; DOT; INSERT; LPAREN; k = expr; COMMA; v = mid; esym = RPAREN; SEMI; p = st      { {Ast.st_structure = Ast.STMapInsert(mp, k, v, p); st_data = Some(($startpos.Lexing.pos_lnum, $startpos.Lexing.pos_cnum - $startpos.Lexing.pos_bol + 1),
                                                                                                                   ($endpos(esym).Lexing.pos_lnum, $endpos(esym).Lexing.pos_cnum - $endpos(esym).Lexing.pos_bol + 1),
                                                                                                                   $startpos.Lexing.pos_fname)} }
    |  v = ID; EQUALS; mp = mid; DOT; DELETE; LPAREN; k = expr; esym = RPAREN; SEMI; p = st      { {Ast.st_structure = Ast.FMapDelete(v, mp, k, p); st_data = Some(($startpos.Lexing.pos_lnum, $startpos.Lexing.pos_cnum - $startpos.Lexing.pos_bol + 1),
                                                                                                                   ($endpos(esym).Lexing.pos_lnum, $endpos(esym).Lexing.pos_cnum - $endpos(esym).Lexing.pos_bol + 1),
                                                                                                                   $startpos.Lexing.pos_fname)} }
    |  v = mid; LARROW; mp = mid; DOT; DELETE; LPAREN; k = expr; esym = RPAREN; SEMI; p = st     { {Ast.st_structure = Ast.STMapDelete(v, mp, k, p); st_data = Some(($startpos.Lexing.pos_lnum, $startpos.Lexing.pos_cnum - $startpos.Lexing.pos_bol + 1),
                                                                                                                   ($endpos(esym).Lexing.pos_lnum, $endpos(esym).Lexing.pos_cnum - $endpos(esym).Lexing.pos_bol + 1),
                                                                                                                   $startpos.Lexing.pos_fname)} }
    |  mp = mid; DOT; esym = CLOSE; SEMI; p = st                                                 { {Ast.st_structure = Ast.MapClose(mp, p); st_data = Some(($startpos.Lexing.pos_lnum, $startpos.Lexing.pos_cnum - $startpos.Lexing.pos_bol + 1),
                                                                                                                   ($endpos(esym).Lexing.pos_lnum, $endpos(esym).Lexing.pos_cnum - $endpos(esym).Lexing.pos_bol + 1),
                                                                                                                   $startpos.Lexing.pos_fname)} }
    |  x = mid; COLON; t = stype; LARROW; MAKECHAN; n = INT; SEMI; p = st     { {Ast.st_structure = Ast.MakeChan(x, t, n, p); st_data = Some(($startpos.Lexing.pos_lnum, $startpos.Lexing.pos_cnum - $startpos.Lexing.pos_bol + 1),
                                                                                                                   ($endpos(n).Lexing.pos_lnum, $endpos(n).Lexing.pos_cnum - $endpos(n).Lexing.pos_bol + 1),
                                                                                                                   $startpos.Lexing.pos_fname)} }
