...
and r_type st = match st with
  ...
  | Tp(tp, r2) :: Tok(T.BAR,_) :: Tok(T.STAR,_) :: Tok(T.LANGLE,r1) :: s -> s $ Tp(A.GetPot(A.Star,tp), join r1 r2)
  ...
  | Tp(tp, r2) :: Tok(T.RANGLE,_) :: Tok(T.STAR,_) :: Tok(T.BAR,r1) :: s -> s $ Tp(A.PayPot(A.Star,tp), join r1 r2)
